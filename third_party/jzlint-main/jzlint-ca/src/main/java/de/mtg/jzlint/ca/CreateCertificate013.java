...
/**
 * Certificates for lints: e_qcstatem_pds_must_have_https_only
 */
public class CreateCertificate013 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        {
            String name = "pdsAllHttps";
            String nameDER = String.format("%s.der", name);
            String namePEM = String.format("%s.pem", name);
            X509Certificate testCertificate =
                    createTestCertificate(privateKey, caIssuerDN, notBefore, new X500Name("CN=test"), "0.4.0.194112.1.1",
                            "https://example.com/en/test.pdf", "https://example.com/de/test.pdf");
        ...
        {
            String name = "pdsOneHttp";
            String nameDER = String.format("%s.der", name);
            String namePEM = String.format("%s.pem", name);
            X509Certificate testCertificate =
                    createTestCertificate(privateKey, caIssuerDN, notBefore, new X500Name("CN=test"), "0.4.0.194112.1.1",
                            "http://example.com/en/test.pdf", "https://example.com/de/test.pdf");
    ...
    private static X509Certificate createTestCertificate(
        ...
        CertificatePolicies qcCertificatePolicies =
                new CertificatePolicies(new PolicyInformation[] {new PolicyInformation(new ASN1ObjectIdentifier(policyOID))});
        ...
        if (!secondURL.isEmpty()) {
            certificateBuilder.addExtension(new Extension(Extension.qCStatements, false, new DEROctetString(qcContent(firstURL, secondURL))));
        }
    ...
    private static DERSequence qcContent(String firstURL, String secondURL) {
        ASN1EncodableVector firstPDSLocation = new ASN1EncodableVector();
        ASN1EncodableVector secondPDSLocation = new ASN1EncodableVector();

        firstPDSLocation.add(new DERIA5String(firstURL));
        firstPDSLocation.add(new DERPrintableString("en"));
        secondPDSLocation.add(new DERIA5String(secondURL));
        secondPDSLocation.add(new DERPrintableString("de"));

        DERSequence firstPDSLocationSeq = new DERSequence(firstPDSLocation);
        DERSequence secondPDSLocationSeq = new DERSequence(secondPDSLocation);

        ASN1EncodableVector pdsLocationsVector = new ASN1EncodableVector();
        pdsLocationsVector.add(firstPDSLocationSeq);
        pdsLocationsVector.add(secondPDSLocationSeq);
        DERSequence pdsLocationsSequence = new DERSequence(pdsLocationsVector);

        ASN1EncodableVector pdsVector = new ASN1EncodableVector();
        pdsVector.add(new ASN1ObjectIdentifier("0.4.0.1862.1.5"));
        pdsVector.add(pdsLocationsSequence);
        DERSequence pdsSeq = new DERSequence(pdsVector);

        ASN1EncodableVector qcVector = new ASN1EncodableVector();
        qcVector.add(pdsSeq);
        return new DERSequence(qcVector);
    }
